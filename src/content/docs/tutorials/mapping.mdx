...
In this tutorial, we'll create our first map.
...
Now we can try testing the game! Navigate to the `Play test` tab. Click into the game and you can use the `arrow` or `WASD` keys to walk around. The ghost in the screen is the player character.

## Map Collisions

TODO: add section on drawing map collisions
