...
First, we need to set up our map with the necessary elements. Add a `tree` and `chest` into your map. You can use the default assets from the `map objects library`. If you haven't already, see the [mapping tutorial](/tutorials/mapping) and [map editor](/features/map-editor) to learn more.
...
2. Click `+ New variable` and select `Switch variable`.
...
3. Add the following events:
   ...
   - `Remove event group`: Remove the "Got the Key" event group at the end.
...
2. Add two conditional events to check if the "Has key" variable is true:
   - **If True**: Add a dialog event with the text “You opened the chest!”
   - **If False**: Add a dialog event with the text “You need the key to open this chest”.
...
TODO: add picture of final event group settings

## Adding Event Groups to the Map

Next, we’ll add our event groups to the map at specific locations.

<Steps>

1. Navigate to `Maps > Map 1 > Events`.
2. Add the “Get key” event group at the tree.
3. Add the “Open chest” event group on the chest.
