...
Now that we have a map, we can add a new character into it! Characters in Pixel Stories are called `Actors`. We'll create them under the `sGame assets` tab

### New Actor

First let's create a barebones actor and add it to the map.
...
To add an animation, we will go back to the actor.
...
To see the animations take place, we will make our new character move.
