...
A **collision** is an invisible rectangle on the map that blocks the player’s movement. Tiles and autotiles do not have built-in collisions, so you add them separately, overlayed in the map. Map objects have their own collision overlay that can be defined in their configuration.
